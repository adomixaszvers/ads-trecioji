...
procedure Kurti(var Medis:TMedis; var klaida:boolean);// var sukurtas_medis :TT);    //sukuriamas medis

function Ar_tuscias( Medis : TMedis):boolean; //tikrinsim ar medis tuscias

procedure Iterpimas_Medis(duomenys : DuomType; var Medis : TMedis; var klaida:boolean); //kuriam vyr. medi su reiksme ir galai nunillinami

procedure RASK_EL(Find_Who : DuomType; Medis : TMedis; var Per_kiek:integer; var klaida :boolean);//iesko per kiek surastu el. (kiek tevu)

procedure Naikinti_Medi(var Medis : TMedis); //sunaikinsim medi

procedure Naikinti_el(var Medis : TMedis; Del_Who : DuomType; var klaida :boolean);    //sunaikinamas ivestas elementas (sutvarkoma sasaja tarp el,o ne numetama)
...
procedure iterpk(duomenys : DuomType; var Medis : T_Lapas; var klaida : boolean);
	...
	begin
            if Medis = nil   //jei pirma el. irasome (jei pradzia = nil)
               ...
               else begin     //jei nepirmas el.
                    tarp := Medis;
                    if Medis^.duom = duomenys then     // jei jau buvo toks elementas ivestas, sustoti
                       begin
                        ...
                        if Medis^.duom > duomenys   //jei naujas duomenys maziau uz pirma, tai i kaire
                         then
                              if Medis^.kaire = nil  //jei ta kaire tuscia -> priskirt reiksme, sukurti toliau
                                 then Medis^.kaire := temp //duodam pag. medziui

                                 else begin //jei uzimta kaire puse, pereinam i kita ir kuriam
                                      Medis := Medis^.kaire;
                                      Iterpk(duomenys, Medis, klaida);
                                      end

                    else if Medis^.duom < duomenys //jei ivesta reiksme didesne (same as with left)
                         then
                             if Medis^.desine = nil     //jei tuscia, idedam
                                then Medis^.desine := temp

                             else begin  //jei uzimta, judam toliau, kuriam
                                  Medis := Medis^.desine;
                                  Iterpk(duomenys, Medis, klaida);
                                  end
                       end;
...
//**************************************************
procedure RASK_EL1(Find_Who : DuomType; Medis : T_Lapas; var Per_kiek :integer; var klaida :boolean);
    var  k : integer; //aukstas pasikeite
          ...
          begin
              ...
              if Medis^.duom = Find_Who     //radom ta el.
                 then begin
                    ...
                    if k = 1 then Per_kiek:= Per_kiek +1;

          end;
procedure RASK_EL(Find_Who : DuomType; Medis : TMedis; var Per_kiek :integer; var klaida :boolean);
          begin
              if Medis.sukurtas = true
				then RASK_EL1(Find_Who, Medis.galva, Per_Kiek, klaida)
				else klaida := true;
          end;
//**************************************************
...
function pirmtakas(Medis : T_Lapas): T_Lapas; //jei tevo desine <> nil, pirmtakas := desine, kitaip -> pirmtakas:= kaire
    var T : T_Lapas;
        begin
            T := Medis^.kaire;      //jei desine butu NILL
            while T^.desine <> nil do  //jei desine <> NILL, priskiriam pati maziausia desiniausia nari T
                  T := T^.desine;
                ...
                then
                     if (Medis^.kaire = nil) or (Medis^.desine = nil)
                        then begin
                             ...
                             if Medis^.kaire = nil    //elementas neturi kaires šakos
                                then  Medis := Medis^.desine    //esamam tevui priskiriam kas yra desinej

                                else  if Medis^.desine = nil    //elementas neturi dešines šakos, SAME AS LEFT
                                          then Medis := Medis^.kaire;  //esamam tevui priskiriam kas yra kairej

                             dispose(T);
                             klaida :=false;
                             end

                        else begin       //elementas turi abi šakas
                                pirm := pirmtakas(Medis);  //surandam saka, kuri pakeis istrinta saka
                                Medis^.duom := pirm^.duom;
                                Naikinti_el1(Medis^.kaire, Medis^.duom, klaida)
                                end

                else    //neradome norimo el, judam toliau pagal < ar >
                    if Del_Who > Medis^.duom then Naikinti_el1(Medis^.desine, Del_Who, klaida)
                                             else Naikinti_el1(Medis^.kaire, Del_Who, klaida)
		...
		begin
			...
			if L_size < R_size then								//jei desine didesne, issiaugom esama teva
				begin											//istrinam teva is medzio
					...
					Naikinti_el1(Medis, Medis^.duom, klaida);	//kadangi desine pasistumi, istrintas el. eis i kaire medzio puse
